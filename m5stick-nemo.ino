...
// -=-=- #define LANGUAGE_EN_US #define LANGUAGE_PT_BR #define LANGUAGE_IT_IT
...
#define NEMO_VERSION "dev"
    ...
    !defined(STICK_C_PLUS) && !defined(STICK_C)
#define STICK_C_PLUS2
#endif
...
//  #define BITMAP M5.Lcd.drawBitmap(0, 0, 320, 240, NEMOMatrix) // This doesn't
//  work, generates static.
...
#define ROTATION
#define USE_EEPROM
#define RTC // TODO: plus2 has a BM8563 RTC but the class isn't the same, needs
            // work.
// #define SDCARD   //Requires a custom-built adapter - Disabled to reduce IRAM
// usage
#define PWRMGMT
...
#define SD_MOSI_PIN 26
#define SD_CS_PIN                                                              \
  14 // can be -1, but sends a lot of messages of error in serial monitor
...
const String contributors[] PROGMEM = {
    "@bicurico",   "@bmorcelli",      "@chr0m1ng", "@doflamingozk",
    "@9Ri7",       "@gustavocelani",  "@imxnoobx", "@klamath1977",
    "@marivaaldo", "@mmatuda",        "@n0xa",     "@niximkk",
    "@unagironin", "@vladimirpetrov", "@vs4vijay"};
...
int apSsidMaxLen = 32;
bool rstOverride =
    false; // Reset Button Override. Set to true when navigating menus.
bool sourApple = false;   // Internal flag to place AppleJuice into SourApple
                          // iOS17 Exploit Mode
bool swiftPair = false;   // Internal flag to place AppleJuice into Swift Pair
                          // random packet Mode
bool androidPair = false; // Internal flag to place AppleJuice into Android Pair
                          // random packet Mode
bool maelstrom =
    false; // Internal flag to place AppleJuice into Bluetooth Maelstrom mode
bool portal_active =
    false; // Internal flag used to ensure NEMO Portal exits cleanly
...
void check_menu_press() {
  ...
  public:
    void setup(MENU *menu, int size, void (*exitCallback)() = nullptr,
    ...
    void setupWithIntro(MENU *menu, int size, String introText,
                        int introDelay = 1000) {
      DISP.fillScreen(BGCOLOR);
      DISP.setCursor(0, 0);
    ...
    void loop() {
      ...
      if (check_select_press()) {
        if (onSelect) {
          ...
          // Custom callbacks may set rstOverride=false to exit, but if still in
          // menu, restore it
          if (!isSwitching) {
            rstOverride = true;
          }
  ...
  void mmenu_setup() { menuController.setup(mmenu, mmenu_size); }
  ...
  int uptime() { return (int(millis() / 1000)); }
  ...
  void screen_dim_proc() {
    if (screen_dim_time > 0) {
      if (screen_dim_dimmed == false) {
        if (uptime() == screen_dim_current ||
            (uptime() + 1) == screen_dim_current ||
            (uptime() + 2) == screen_dim_current) {
          screenBrightness(0);
  ...
  /// Dimmer MENU ///
  MENU dmenu[] = {
      {TXT_BACK, screen_dim_time}, {TXT_NEVER, 0},
      {("5 " TXT_SEC), 5},         {("10 " TXT_SEC), 10},
      {("15 " TXT_SEC), 15},       {("30 " TXT_SEC), 30},
      {("60 " TXT_SEC), 60},       {("120 " TXT_MIN), 120},
      {("240 " TXT_MIN), 240},
  };
  ...
  MENU cmenu[] = {
      {TXT_BACK, 0},         {TXT_BLACK, 1},    {TXT_NAVY, 2},
      {TXT_DARKGREEN, 3},    {TXT_DARKCYAN, 4}, {TXT_MAROON, 5},
      {TXT_PURPLE, 6},       {TXT_OLIVE, 7},    {TXT_LIGHTGREY, 8},
      {TXT_DARKGREY, 9},     {TXT_BLUE, 10},    {TXT_GREEN, 11},
      {TXT_CYAN, 12},        {TXT_RED, 13},     {TXT_MAGENTA, 14},
      {TXT_YELLOW, 15},      {TXT_WHITE, 16},   {TXT_ORANGE, 17},
      {TXT_GREENYELLOW, 18}, {TXT_PINK, 19},
  };
  ...
  MENU thmenu[] = {
      {TXT_BACK, 0}, {"Nemo", 1},      {"Tux", 2},           {"Bill", 3},
      {"Steve", 4},  {"Lilac", 5},     {"YellowOnBlack", 6}, {"NightShift", 7},
      {"Camo", 8},   {"BubbleGum", 9}, {"Contrast", 10},     {TXT_COLOR, 99},
  };
  ...
  int get_battery_voltage() { return M5.Power.getBatteryLevel(); }
  ...
  void tvbgone_setup() {
    ...
    // Hack: Set IRLED high to turn it off after setup. Otherwise it stays on
    // (active low)
  ...
  void sendAllCodes() {
    bool endingEarly = false; // will be set to true if the user presses the
                              // button during code-sending
    if (region == NA) {
  ...
  void timeset_loop() {
    ...
    StickCP2.Rtc.setDateTime(
        {{dt.date.year, dt.date.month, dt.date.date}, {hour, minute, 0}});
#else
  ...
  /// BTSPAM MENU ///
  MENU btmenu[] = {
      {TXT_BACK, 5},       {"AppleJuice", 0}, {"Swift Pair", 1},
      {"Android Spam", 4}, {TXT_SA_CRASH, 2}, {"BT Maelstrom", 3},
      {"BLE Hunter", 25},
  };
  ...
  void aj_adv_setup() { rstOverride = false; }
  ...
  void aj_adv() {
    ...
    if (millis() > advtime + ajDelay) {
      ...
      if (sourApple) {
        ...
        const uint32_t model =
            android_models[rand() % android_models_count].value; // Action Type
    ...
    if (check_next_press()) {
      ...
      maelstrom = false;
      pAdvertising
          ->stop(); // Bug that keeps advertising in the background. Oops.
      delay(250);
    }
  }
  ...
  void wifispam_setup() {
    ...
    switch (spamtype) {
    case 1:
      for (str = funnyssids; *str; ++str)
        ct += *str == '\n';
      DISP.printf(" - %d SSIDs:\n", ct);
      DISP.print(funnyssids);
      break;
    case 2:
      for (str = rickrollssids; *str; ++str)
        ct += *str == '\n';
      DISP.printf(" - %d SSIDs:\n", ct);
  ...
  /// WIFI MENU ///
  MENU wsmenu[] = {
      {TXT_BACK, 5},       {TXT_WF_SCAN, 0},     {TXT_WF_SPAM_FUN, 1},
      {TXT_WF_SPAM_RR, 2}, {TXT_WF_SPAM_RND, 3}, {"NEMO Portal", 4},
  };
  ...
  void wscan_result_loop() {
    ...
    if (check_select_press()) {
      ...
      if (check_select_press()) {
        ...
        channel =
            static_cast<uint8_t>(WiFi.channel(cursor)); // WiFi - save channel
        rstOverride = false;
        current_proc = 20;
        isSwitching = true;
        delay(100);
...
// Boot Screen
  ...
  void setup() {
    ...
    if (EEPROM.read(0) > 3 || EEPROM.read(1) > 240 || EEPROM.read(2) > 100 ||
        ...
        EEPROM.read(6) > 100) {
      // Assume out-of-bounds settings are a fresh/corrupt EEPROM and write
      // defaults for everything
      Serial.println(
          "EEPROM likely not properly configured. Writing defaults.");
  ...
  void menu_controller_loop() { menuController.loop(); }
  ...
  // RSSI visualization functions (used by BLE Hunter)
  ...
  void draw_rssi_bar(float rssi, float rssimax = -20) {
    ...
    if (remainingW > 0) {
      DISP.fillRect(barX + 2 + fillW, barY + 2, remainingW, barH - 4,
                    TFT_BLACK);
    }
  }
  ...
  void add_unique_ble_device(const String &mac, const String &name,
                             int32_t rssi) {
  ...
  void ble_hunter_loop() {
    ...
    if (ble_scanner && (now - last_scan > 1000)) {
      ble_scanner->start(
          BLE_SCAN_TIME,
          false); // Scan for BLE_SCAN_TIME seconds, don't restart
      last_scan = now;
    }
    ...
    if (bh_pkts && !ble_channel_hop_pause &&
        ble_stats.total_devices > bh_pkts) {
      play_alert_beep();
    }
    ...
    if (ble_stats.rssi_count > 0) {
      DISP.print("RSSI:");
      draw_rssi_bar(ble_stats.avg_rssi,
                    bh_max_rssi); // Using existing RSSI bar function
    }
  }
